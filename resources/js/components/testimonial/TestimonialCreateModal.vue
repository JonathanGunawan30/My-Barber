                    ...
                    <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center space-x-3">
                            <div class="p-2 bg-blue-100 dark:bg-blue-900/50 rounded-lg">
                                <MessageSquareText class="w-6 h-6 text-blue-600 dark:text-blue-400" />
                            </div>
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Add New Testimonial</h2>
                                <p class="text-sm text-gray-600 dark:text-gray-400">Create a new customer testimonial</p>
                            </div>
                        </div>
                        <button
                            @click="closeModal"
                        ...
                        <form @submit.prevent="submitForm" class="space-y-6">
                            <div>
                                ...
                                <div class="relative">
                                    <select
                                        ...
                                        :class="{ 'border-red-500 focus:ring-red-500': errors.customer_id }"
                                    >
                                        <option value="" disabled>-- Select a Customer --</option>
                                        <option v-for="customer in customers" :key="customer.id" :value="customer.id">{{ customer.name }} ({{ customer.email }})</option>
                                    </select>
                                    <Users class="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
                                </div>
                                <p v-if="errors.customer_id" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.customer_id }}</p>
                            </div>

                            <div>
                                <label for="barber_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Barber *
                                </label>
                                <div class="relative">
                                    <select
                                        ...
                                        required
                                        class="appearance-none w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors duration-200"
                                        :class="{ 'border-red-500 focus:ring-red-500': errors.barber_id }"
                                    >
                                    ...
                                    <User class="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
                                </div>
                                <p v-if="errors.barber_id" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.barber_id }}</p>
                            </div>
                            ...
                            <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                ...
                                >
                                    ...
                                    <Plus class="w-4 h-4" />
                                    <span>{{ isSubmitting ? 'Creating...' : 'Create Testimonial' }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
...
import { MessageSquareText, Star, User, Users, Plus, X, Loader2 } from 'lucide-vue-next'
...
const submitForm = async () => {
    ...
    if (!form.value.barber_id) {
        errors.value.barber_id = 'Barber is required.'
        return
    }
