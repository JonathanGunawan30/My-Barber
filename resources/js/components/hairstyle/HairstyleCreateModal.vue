                ...
                >
                    ...
                    <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex items-center space-x-3">
                            <div class="p-2 bg-blue-100 dark:bg-blue-900/50 rounded-lg">
                                <Palette class="w-6 h-6 text-blue-600 dark:text-blue-400" />
                            </div>
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Add New Hairstyle</h2>
                                <p class="text-sm text-gray-600 dark:text-gray-400">Define a new hairstyle for your barbershop</p>
                            </div>
                        </div>
                        <button
                            @click="closeModal"
                    ...
                    <div class="p-6 overflow-y-auto">
                        <form @submit.prevent="submitForm" class="space-y-6">
                            <!-- Hairstyle Name -->
                            <div>
                                <label for="hairstyle-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Hairstyle Name *
                                </label>
                                <div class="relative">
                                    <input
                            ...
                            <!-- Description -->
                            <div>
                                <label for="hairstyle-description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Description *
                                </label>
                                <div class="relative">
                                    <textarea
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                <label for="is_featured" class="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center">
                                    <Star class="w-5 h-5 mr-2 text-yellow-500" />
                                    Mark as Featured
                                </label>
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" v-model="form.is_featured" id="is_featured" class="sr-only peer">
                            ...
                            <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                ...
                                >
                                    ...
                                    <Plus class="w-4 h-4" />
                                    <span>{{ isSubmitting ? 'Creating...' : 'Create Hairstyle' }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
...
import { Palette, MessageSquareText, Upload, Star, Plus, X, Loader2 } from 'lucide-vue-next'
...
const submitForm = async () => {
    ...
    if (!form.value.photo) {
        errors.value.photo = 'Hairstyle photo is required.'
        return
    }
    ...
    try {
        ...
        router.post('/admin/hairstyles', formData, {
            forceFormData: true,
            onSuccess: () => {
                resetForm()
                emit('created')
