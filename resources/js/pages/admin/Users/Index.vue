            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ inactiveUsersCount }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div v-if="filteredUsers.length === 0" class="p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
                    {{ searchQuery || statusFilter ? 'No users found' : 'No users yet' }}
                </h3>
                <p class="text-gray-600 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                    {{ searchQuery || statusFilter ? 'Try adjusting your search criteria or filters.' : 'Start by creating your first user profile.' }}
                </p>
                <button
                    v-if="!searchQuery && !statusFilter"
                    @click="openCreateModal"
            ...
            <div v-if="totalPages > 1" class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span class="font-medium">{{ Math.min(currentPage * parseInt(perPage), filteredUsers.length) }}</span> of
...
import {
    Plus, Search, User, Users, CircleCheck, XCircle, Eye, Pencil, Trash, X,
...
const totalPages = computed(() => {
    return Math.ceil(filteredUsers.value.length / parseInt(perPage.value))
})
...
const inactiveUsersCount = computed(() => {
    return props.users.filter(user => user.status === 'inactive').length
})
...
const handleUserCreated = () => {
    router.reload({
        ...
        onError: (errors) => {
            console.error('Error reloading data after user creation:', errors);
        }
    })
}
            ...
            onSuccess: (page) => {
                if (page.props.flash?.error) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: `User status has been successfully ${action}d.`,
                        confirmButtonColor: '#3B82F6',
                        timer: 2000,
                        timerProgressBar: true
                    })
