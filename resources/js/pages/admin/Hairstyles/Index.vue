<template>
    <Layout>
        ...
        <div class="mb-8">
                ...
                <div class="flex-1 max-w-md">
                    <div class="relative group">
                        ...
                        <input
                            v-model="searchQuery"
                            type="text"
                            placeholder="Search hairstyles by name or description..."
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <option value="false">Not Featured Only</option>
                        </select>
                    ...
                    >
                        <button
                            v-if="searchQuery || featuredFilter"
                            @click="clearFilters"
            ...
            <div v-if="searchQuery || featuredFilter" class="mt-4 flex items-center gap-2">
                ...
                <div class="flex gap-2">
                    ...
                    <span v-if="featuredFilter" class="inline-flex items-center px-3 py-1 bg-purple-100 dark:bg-purple-900/50 text-purple-800 dark:text-purple-300 text-sm rounded-full">
                        Featured: {{ featuredFilter === 'true' ? 'Yes' : 'No' }}
                        <button @click="featuredFilter = ''" class="ml-2 hover:text-purple-600">
                            <X class="w-3 h-3" />
                        </button>
                    </span>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Hairstyles</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ hairstyles.length }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Featured Hairstyles</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ featuredHairstylesCount }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Hairstyles</h3>
                ...
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Showing {{ paginatedHairstyles.length }} of {{ filteredHairstyles.length }} results
                </div>
            </div>

            <div v-if="filteredHairstyles.length === 0" class="p-12 text-center">
                <div class="inline-flex items-center justify-center w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-full mb-4">
                    <Palette class="w-8 h-8 text-gray-400" />
                </div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
                    {{ searchQuery || featuredFilter ? 'No hairstyles found' : 'No hairstyles yet' }}
                </h3>
                <p class="text-gray-600 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                    {{ searchQuery || featuredFilter ? 'Try adjusting your search criteria or filters.' : 'Start by adding your first hairstyle.' }}
                </p>
                <button
                    v-if="!searchQuery && !featuredFilter"
                    @click="openCreateModal"
            ...
            <div v-else class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Hairstyle Info</th>
                    ...
                    <tr
                        v-for="(hairstyle, index) in paginatedHairstyles"
                        :key="hairstyle.id"
                    ...
                    >
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ hairstyle.name }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs">{{ hairstyle.description }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div v-if="hairstyle.photo" class="flex items-center">
                                <img
                                    ...
                                    @click="showImageModal(hairstyle)"
                                />
                                <button
                                    @click="showImageModal(hairstyle)"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <button
                                @click="toggleFeatured(hairstyle)"
                                :class="[
                                        ...
                                        hairstyle.is_featured
                                          ...
                                          : 'bg-gray-100 text-gray-800 dark:bg-gray-700/50 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-900/70'
                                      ]"
                            >
                                <div :class="[
                                        'w-1.5 h-1.5 rounded-full mr-1.5',
                                        hairstyle.is_featured ? 'bg-yellow-500' : 'bg-gray-500'
                                      ]"></div>
                                {{ hairstyle.is_featured ? 'Featured' : 'Not Featured' }}
                            </button>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                <button
                                    @click="viewHairstyle(hairstyle)"
            ...
            <div v-if="totalPages > 1" class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span class="font-medium">{{ Math.min(currentPage * parseInt(perPage), filteredHairstyles.length) }}</span> of
                        <span class="font-medium">{{ filteredHairstyles.length }}</span> results
                    </div>
            ...
            >
                ...
                >
                    <div v-if="showModal" class="relative max-w-4xl max-h-full">
                        <img
                            :src="`/storage/${selectedHairstyle.photo}`"
                        ...
                        <div class="absolute bottom-4 left-4 bg-black bg-opacity-75 text-white p-3 rounded-lg">
                            <h3 class="font-medium">{{ selectedHairstyle.name }}</h3>
                            <p class="text-sm text-gray-300">{{ selectedHairstyle.description }}</p>
                        </div>
                    </div>
                </Transition>
            </div>
        ...
        <!-- Create Hairstyle Modal -->
        <HairstyleCreateModal
            :is-open="showCreateModal"
            @close="closeCreateModal"
            @created="handleHairstyleCreated"
        />

        <!-- Edit Hairstyle Modal -->
        <HairstyleEditModal
            :is-open="showEditModal"
            :hairstyle="selectedHairstyleForEdit"
            @close="closeEditModal"
            @updated="handleHairstyleUpdated"
        />

        <!-- Detail Hairstyle Modal -->
        <HairstyleDetailModal
            :is-open="showDetailModal"
            :hairstyle="selectedHairstyleForDetail"
...
import HairstyleEditModal from '@/components/hairstyle/HairstyleEditModal.vue'
import HairstyleDetailModal from '@/components/hairstyle/HairstyleDetailModal.vue'
import { router } from '@inertiajs/vue3'
import {
    Plus, Search, Palette, Star, Eye, Pencil, Trash, Image, X,
...
const selectedHairstyleForEdit = ref(null)
const showDetailModal = ref(false)
const selectedHairstyleForDetail = ref(null)
const currentPage = ref(1)
const perPage = ref('10')

const filteredHairstyles = computed(() => {
    let filtered = props.hairstyles

    if (searchQuery.value) {
        filtered = filtered.filter(hairstyle =>
            hairstyle.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            hairstyle.description.toLowerCase().includes(searchQuery.value.toLowerCase())
        )
    }

    if (featuredFilter.value !== '') {
        const filterValueBoolean = featuredFilter.value === 'true';

        filtered = filtered.filter(hairstyle => {
            const hairstyleIsFeaturedBoolean = Boolean(hairstyle.is_featured);
            return hairstyleIsFeaturedBoolean === filterValueBoolean;
        });
    }
...
const totalPages = computed(() => {
    return Math.ceil(filteredHairstyles.value.length / parseInt(perPage.value))
})

const paginatedHairstyles = computed(() => {
    const start = (currentPage.value - 1) * parseInt(perPage.value)
    const end = start + parseInt(perPage.value)
    return filteredHairstyles.value.slice(start, end)
})
...
const featuredHairstylesCount = computed(() => {
    return props.hairstyles.filter(hairstyle => hairstyle.is_featured).length
})
...
const showImageModal = (hairstyle) => {
    ...
    router.reload({
        ...
        onSuccess: () => {
            ...
            Swal.fire({
                ...
                text: 'New hairstyle has been added successfully.',
        ...
        onError: (errors) => {
            console.error('Error reloading data after hairstyle creation:', errors);
        }
    })
}

const viewHairstyle = (hairstyle) => {
    selectedHairstyleForDetail.value = hairstyle
    showDetailModal.value = true
}

const closeDetailModal = () => {
    showDetailModal.value = false
    selectedHairstyleForDetail.value = null
}

const handleEditFromDetail = (hairstyle) => {
    closeDetailModal()
    selectedHairstyleForEdit.value = hairstyle
    showEditModal.value = true
}

const editHairstyle = (hairstyle) => {
    selectedHairstyleForEdit.value = hairstyle
    showEditModal.value = true
}
...
const closeEditModal = () => {
    ...
    selectedHairstyleForEdit.value = null
}

const handleHairstyleUpdated = () => {
    router.reload({
        only: ['hairstyles'],
        onSuccess: () => {
            closeEditModal()
            Swal.fire({
                ...
                text: 'Hairstyle information has been updated successfully.',
                confirmButtonColor: '#3B82F6',
                timer: 2000,
                timerProgressBar: true
            });
        ...
        onError: (errors) => {
            console.error('Error reloading data after hairstyle update:', errors);
        }
    })
}

const toggleFeatured = async (hairstyle) => {
    const newFeaturedStatus = !hairstyle.is_featured;
    const action = newFeaturedStatus ? 'feature' : 'unfeature';

    const result = await Swal.fire({
        title: `${action.charAt(0).toUpperCase() + action.slice(1)} Hairstyle?`,
        text: `Are you sure you want to ${action} "${hairstyle.name}"?`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: newFeaturedStatus ? '#F59E0B' : '#6B7280',
    ...
    if (result.isConfirmed) {
        router.patch(`/admin/hairstyles/${hairstyle.id}/toggle-featured`, {
            ...
            onSuccess: () => {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: `Hairstyle has been ${action}d successfully.`,
                    confirmButtonColor: '#3B82F6',
                    timer: 2000,
                    timerProgressBar: true
                })
            ...
            onError: () => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: `Something went wrong while ${action}ing the hairstyle.`,
                    confirmButtonColor: '#3B82F6'
                })
            }
        })
...
const confirmDelete = async (hairstyle) => {
    const result = await Swal.fire({
        title: 'Delete Hairstyle?',
        text: `Are you sure you want to delete "${hairstyle.name}"? This action cannot be undone.`,
        icon: 'warning',
        showCancelButton: true,
    ...
    if (result.isConfirmed) {
        router.delete(`/admin/hairstyles/${hairstyle.id}`, {
            onSuccess: () => {
                Swal.fire({
                    icon: 'success',
                    title: 'Deleted!',
                    text: 'Hairstyle has been deleted successfully.',
                    confirmButtonColor: '#3B82F6',
                    timer: 2000,
                    timerProgressBar: true
                })
            ...
            onError: () => {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Something went wrong while deleting the hairstyle.',
                    confirmButtonColor: '#3B82F6'
                })
            }
        })
...
watch([searchQuery, featuredFilter, perPage], () => {
    currentPage.value = 1
})
</script>
